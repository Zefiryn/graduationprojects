...
$(document).ready(function () {
    ...
    if ($("#regulation").length > 0) {
        sortElements('#left, #right', "/regulation/sort/", function () {
            sortNumbers($("#regulation"), "&sect; ", "");
        }, true);
    }

    if ($("#faq").length > 0) {
        sortElements('#left, #right', "/faq/sort/", function (id) {
            sortNumbers($("#faq"), "", ". ");
        }, true);
    }

    if ($("#diploma_gallery").length > 0) {
        sortColumnElements('#diploma_gallery', function (event, ui) {
            sortDiplomaImages(event, ui, "/diploma/sort/");
        }, false);
    }

    if ($("#newsFiles").length > 0) {
        sortColumnElements('#newsFiles', function (event, ui) {
            sortNewsFiles(event, ui, "/news/sort");
        });
    }

    if ($("#press_files").length > 0) {
        sortColumnElements('#press_files', function (event, ui) {
            sortPressFiles(event, ui, "/press/sort/");
        }, false);
    }
...
function voting() {
    $('p.votes span.vote, p.votes span.admin_vote').on('click', function () {
        var self = $(this);
